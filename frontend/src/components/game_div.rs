use crate::{ai::mct::MakeMoveTask, components::RegionDiv};
...
pub(crate) fn ai_game_div() -> Html {
    ...
    let ai_task = use_oneshot_runner::<MakeMoveTask>();
        ...
        Callback::from(
            move |(region_index, tile_index): (BoardIndex, BoardIndex)| {
                ...
                spawn_local(async move {
                    let (region_index, tile_index) = ai_agent.run(game.clone()).await;
...
pub(crate) fn lm_game_div() -> Html {
    ...
    let callback = {
        let state = game.clone();
        Callback::from(move |(region_index, tile_index)| {
            let mut game = (*state).clone();
            if matches!(
                game.mark_tile(region_index, tile_index),
                MarkTileResult::NoChange
